...
export default {
  ...
  tagLanVlanBody:     '每个 LAN 端口都变为<strong>中继（trunk）端口</strong>，而不是 LAN VLAN 上的普通接入（access）端口（不带标记/PVID）。中继端口要求每个连接的设备都支持 VLAN，例如受管交换机，或在以太网口上配置了 VLAN 的 Linux/Mac。',
  ...
  wiredBackhaulNote:  '尽可能优先使用有线回程而非无线。',
