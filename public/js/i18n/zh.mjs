...
export default {
  ...
  batmanAdvHelp: '为纯无线网状网络提供更好的多跳路由。跳数很少，或有线与无线链路混用时，使用启用 STP 的普通 802.11s 更好。',
  batmanAllVlan: 'WAN-B 与额外 VLAN 经 BATMAN-adv',
  batmanAllVlanHelp: '默认仅 LAN/访客/IoT/VPN/WAN VLAN 经 bat0 传输。',
  backhaulOnly: '关闭所有 AP 接口',
  disableAllAps: '适用于仅路由或仅 mesh 回程的节点。',
  bridgeStp:          '为 <code>br-vlan</code> 启用 STP（生成树协议）',
  stpNote:            '在混合有线和无线回程时防止二层环路。',
  nonCtAth10k:        '使用非 CT ath10k',
  buildFirmware:      '构建固件',
  ...
  ifaceReserved:    '接口名称 "{field}" 已被 wrtnova.sh 保留，请换一个。',
  ifaceDup:         '接口名称 "{field}" 已被其他网络使用。',
  ...
  sshPasswordAuth:    'SSH 认证',
  sshPasswordAuthHelp: '仅密钥将拒绝 SSH 密码登录。不影响 LuCI 网页登录。',
  sshAuthOn:          '密钥 + 密码',
  sshAuthOff:         '仅密钥',
  timezone:           '时区',
  timeFormat:         '时间格式',
  ...
  timeFormat24:       '24 小时',
  timeFormat12:       '12 小时',
  timeFormatHelp:     '需要 OpenWrt 25 或更高版本。',
  networkSection:     '网络',
  ...
  wiredBackhaulNote:  '提示：条件允许时，有线回程始终优于无线。',
  wifiDefaultNote:    '密码留空则使用默认值：12345678',
  wifiRoamingNote:    '节点间的 SSID 和密码必须一致，才能实现无缝漫游。',
  wifiDefaultBlank:   '密码留空则使用默认值：12345678',
  wifiPskVlanNote:    '每个 VLAN 独立 PSK 需要为每个启用的网络设置唯一密码，以引导客户端。不支持 AP/VLAN 的射频会回退为独立的 SSID。',
  ...
  bandSuffix:         '添加频段后缀',
  bandSuffixHelp:     '在每个 SSID 末尾添加 2G/5G/6G。这可能导致 <code>usteer</code> 的频段引导失效；同频段的 AP 之间漫游不受影响。',
  indexSuffix:        '添加 AP 索引后缀',
  indexSuffixHelp:    '在每个 SSID 后追加 AP 索引（仅 AP 模式）。这可能导致 802.11r 和 802.11k/v 在多个 AP 之间失效。',
  pskVlan:            '按 VLAN 分配 PSK（Multi-PSK）',
  ...
  adguardRamNote:     '在内存小于 110 MB 的设备上，<code>wrtnova.sh</code> 会跳过 AdGuard Home 的初始设置。',
  ...
  dnsNone:            '无',
  dnsNoneHelp:        '仅使用纯 dnsmasq。无广告拦截，无加密 DNS，占用最小。',
  adguardMainDns:     '将 AdGuard Home 作为主 DNS 解析器',
  adguardMainDnsHelp: '将 AdGuard Home 绑定到 53 端口，作为主 DNS 解析器。',
  adgQueryLog:        'AdGuard Home 查询日志',
  adgQueryLogHelp:    '在 AdGuard Home 的查询日志中记录 DNS 查询。会占用大量内存。',
  adgSafeSearch:      'AdGuard Home 安全搜索',
  adgSafeSearchHelp:  '在以下搜索引擎中强制启用安全搜索：Google、YouTube、Bing、DuckDuckGo、Ecosia、Yandex、Pixabay。',
  ...
  blockDotDoqHelp:    '阻止客户端通过 DoT/DoQ 绕过广告拦截和过滤。',
  blockDoh:           '阻断公共的基于 HTTPS 的 DNS 服务器',
  blockDohHelp:       '通过 banIP 阻断已知的 DNS-over-HTTPS 解析器，让客户端更难绕过广告拦截和过滤。',
  forceDns:           '强制 DNS 指向路由器',
  forceDnsHelp:       '将 LAN、访客和 IoT 客户端的所有 53 端口 (TCP/UDP) DNS 查询重定向到本路由器的 DNS。指向 8.8.8.8 或 1.1.1.1 的客户端会被路由器透明地接管。',
  firewall:           '防火墙',
  ...
  banipMemoryNote:        '每个启用的订阅源或国家/地区都会增加内存占用。启用过多可能迅速耗尽内存并触发 OOM killer。',
