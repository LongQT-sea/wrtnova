...
export function mergeNodeConfig(sharedConfig, nodeOverrides) {
  ...
  return {
    ...
    IOT_BASE_PREFIX:   iotOn   ? (c.IOT_BASE_PREFIX   || '') : '', IOT_IFACE:   iotOn   ? (c.IOT_IFACE   || '') : '', IOT_VLAN_ID:   vlan.IOT_VLAN_ID, IOT_SUBNET:   iotOn   ? (c.IOT_SUBNET   || '') : '',
    ...
    COUNTRY_CODE: c.COUNTRY_CODE || '', DENSE_ENV: flag(c.DENSE_ENV), WIRELESS_MESH: flag(c.WIRELESS_MESH), WIFI_KVR: flag(c.WIFI_KVR), PSK_VLAN: flag(c.PSK_VLAN), GUEST_ISOLATE: guestOn ? flag(c.GUEST_ISOLATE) : '',
    ...
    DOH_UPSTREAMS:    (c.DNS_MODE || 'adguardhome') !== 'none' ? (c.DOH_UPSTREAMS || '') : '',
    BOOTSTRAP_DNS:    (c.DNS_MODE || 'adguardhome') !== 'none' ? (c.BOOTSTRAP_DNS || '') : '',
