(function () {
    ...
    bcryptHash(rootPasswd).then(adguardHash => {
      ...
      .then(({ ok, status, data }) => {
        if (!ok) throw new Error(
          [data.error, data.detail, data.message].filter(Boolean).join(' — ') || 'HTTP ' + status
        );
        if (data.firmware_url) { finishNodeBuild(net, node, panelActEl(node.id) || actEl, data, activeAsu); return; }
        if (!data.request_hash) throw new Error('Unexpected response from /api/build');
        pollNodeBuild(net, node, panelActEl(node.id) || actEl, data.request_hash, data.asu_url || activeAsu);
      })
  ...
  async function startBuildAllNode(net, node, onComplete) {
    ...
    if (resp.firmware_url) {
      finishBuildAllNode(net, node, resp, activeAsu);
      onComplete();
      return;
    }
    if (!resp.request_hash) {
      updateBuildAllRow(node.id, null, 'Unexpected response');
      onComplete();
      return;
    }
    pollBuildAllNode(net, node, resp.request_hash, resp.asu_url || activeAsu, onComplete);
  }
